...
def test_degrees_go_in_and_slopes_are_stored(library_path):
    ...
    assert run("condition", library_path, "--id", "heeled", "--z-origin", -4.0, "--heel", 5.0) == 0

    with Library.open(library_path) as library:
        condition = library.condition("heeled")
        assert condition.heel == pytest.approx(np.tan(np.radians(5.0)))
        assert condition.heel != pytest.approx(5.0), "degrees must never reach storage"
...
def test_an_out_of_domain_condition_is_refused_with_a_reason(library_path, capsys):
    assert run("condition", library_path, "--z-origin", -4.0, "--heel", 89.999) == 1
